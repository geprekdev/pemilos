...
@section('content')
  ...
  <form action="{{ route('admin.settings.update', ['setting' => $setting->id]) }}" method="POST"
    ...
    <input
      ...
      name="attribute" type="text" id="attribute" value="{{ old('attribute', $setting->attribute) }}">
    @error('attribute')
    ...
    <input
      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500"
      name="value" type="text" id="value" value="{{ old('value', $setting->value) }}">
    @error('value')
      <span class="text-xs text-red-600 dark:text-red-400">
        {{ $message }}
      </span>
    ...
    <button type="submit" onclick="return window.confirm('Yakin ingin mengedit setting ini?')"
