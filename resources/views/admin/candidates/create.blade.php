...
@section('content')
  ...
  <form action="{{ route('admin.candidates.store') }}" method="POST" enctype="multipart/form-data"
    ...
    <div class="mt-4 text-sm">
      ...
      <div class="mt-2">
        <label class="inline-flex items-center mx-3 text-gray-600 dark:text-gray-400">
          <input type="radio" name="label" value="MPK" {{ old('label') === 'MPK' ? 'checked' : '' }} />
          <span class="ml-2">MPK</span>
        </label>
        <label class="inline-flex items-center mx-3 text-gray-600 dark:text-gray-400">
          <input type="radio" name="label" value="OSIS" {{ old('label') === 'OSIS' ? 'checked' : '' }} />
    ...
    <input min="1"
      class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-purple-500 dark:focus:border-purple-500"
      name="number" type="number" id="number" value="{{ old('number') }}">
    @error('number')
      <span class="text-xs text-red-600 dark:text-red-400">
        {{ $message }}
      </span>
    ...
    <button type="submit" onclick="return window.confirm('Yakin ingin menambah kandidat ini?')"
